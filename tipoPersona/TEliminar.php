...
<html lang="es">
	...
	<body background="super2.jpg">
		...
		<center>
		<fieldset>
			<h1>Eliminar a un tipo de persona</h1>
			<center>
			<form action="TEliminar.php" method="post">
				Ingrese el Codigo de la persona a eliminar:
					...
					if ($reg=mysqli_fetch_array($registros))
					{
						if($reg['Sistema_Principal']=='UPW001')
						{
							...
							else
							{
								...
								if ($reg = mysqli_fetch_array($registros))
								{
									mysqli_query($conexion,"delete from tipo_personas where Codigo_Tipo_Persona = '$cod'") or die("Problemas en el Select...".mysqli_error($conexion));
